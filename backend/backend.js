...
// - changed id type from integer to string since hexadecimal passed from front-end is a string
...
app.post('/todos', authenticateUser, (req, res) => {
  ...
  const newTodo = {
    //id: todos[user.username].length + 1,                    // This is a problem!
    // when delete a task, new task added will have the same id as previous one
    // let's try randomize

    id: crypto.randomBytes(16).toString('hex'), // Generate a random unique ID
    title,
    description,
    completed: false,
...
const PORT = 3000;        // USE this port for the backend server!!
app.listen(PORT, () => {
